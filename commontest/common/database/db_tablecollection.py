...
class DbTableCollection:
    ...
    def update_value_to_all_records(self, table_nameinx, col_nameinx, value: str):
        ...
        for idx in idx_list:
            ...
            if not result:
                self.glogger.info("Failed to update idx=%s table=%s col=%s value=%s", idx, table_nameinx, col_nameinx,
                                  value)
            else:
                count_updates += 1
    ...
    # --------------------------------------------------------------------
    def get_all_rows(self, table_nameinx) -> list:
        dbtable = self.get_table_handle(table_nameinx)
        if dbtable is None: return None
        return dbtable.get_all_rows()
    ...
    # --------------------------------------------------------------------
    # Get values of a table by two columns
    # --------------------------------------------------------------------
    def get_rows_specific_two_col_value(self, table_nameinx, col_name1, value1, col_name2, value2) -> list:
        dbtable = self.get_table_handle(table_nameinx)
        if dbtable is None: return None
        return dbtable.get_rows_specific_two_col_value(col_name1, value1, col_name2, value2)
    ...
    def get_table_row_values(self, idx: str, table_nameinx) -> dict:
        ...
        self.glogger.info("Table %s idx=%s Value=%s Type=%s", table_nameinx, idx, dict_values, str(type(dict_values)))
        return dict_values
    ...
    def get_all_tables(self):
        ...
        for tablename in self.initial_tables_names:
            rows_list = self._get_table_data(tablename)
            self.glogger.debug("Table:%s rows_list=%s", tablename, rows_list)
            all_dbitems_tables.append(rows_list)
        self.glogger.info("all_items=%s",all_dbitems_tables)
        return all_dbitems_tables

    # --------------------------------------------------------------------
    # Get all changes feed of the tables
    # --------------------------------------------------------------------
    def get_all_change_feeds(self, start_date_time):
        if not dbwrapper_get_db_session():
            self.glogger.error("There is no connection to the database")
            return None
        all_changesfeed = list()
        for tablename in self.initial_tables_names:
            dbtable = self.get_table_handle(tablename)
            if dbtable is None: continue
            rows_list = dbtable.get_change_feeds(start_date_time)
            self.glogger.debug("Table:%s rows_list=%s", tablename, rows_list)
            all_changesfeed.append(rows_list)
        self.glogger.info("all_changesfeed=%s", all_changesfeed)
        return all_changesfeed
    ...
    def _create_tables(self):
        ...
        self.glogger.debug("Creating TableCollection (%d) tables : %s", self._count_initial_tables(),
                           self._count_initial_columns_count())
    ...
    def __create_tables(self):
        for table_name in self.initial_tables_names:
            ...
            self.glogger.debug("Creating tablecollection %s with %s colums, with table_attribute=%s", table_name,
                               colinfo, table_attribute)
    ...
    def _get_table_data(self, tablenameinx) -> list:
        ...
        rows_list = tbl.get_all_rows()
        #rows_list = tbl.convert_rows_to_dict_list(rows_list) # Already done before
        return rows_list
    ...
    def get_table_data_per_colnames(self, tablenameinx, colnames: list) -> list:
        try:
            ...
            self.glogger.error("failed to get data of the colmns:%s err=%s", colnames, err)
            return None
    ...
    def get_table_handle(self, table_nameinx) -> DbTable:
        ...
        if not table_nameinx in self.initial_tables_names:
            self.glogger.error("table %s not found in the list", str(table_nameinx))
            return None
        inx = self.initial_tables_names.index(table_nameinx)
        if inx >= len(self.dbtables):
            self.glogger.error("The table inx %s is out of range (%d), table_nameinx=%s", inx, len(self.dbtables),
                               str(table_nameinx))
            return None
    ...
    # --------------------------------------------------------------------
    # Add a colum to the table(s)
    # If no table name passed (None), it is added to all tables
    # --------------------------------------------------------------------
    def add_new_column(self, tablenameinx, colname: str):
        if tablenameinx is not None:
            tbl = self.get_table_handle(tablenameinx)
            if tbl is None: return False
            return self._add_new_column(tablenameinx, colname)

        for tbl in self.dbtables:
            if not self._add_new_column(tbl, colname):
                return False
        return True
    ...
    # Add a colum to the table(s)
    # --------------------------------------------------------------------
    def _add_new_column(self, dbtable, colname: str):
        try:
            new_colname = dbtable.add_new_column(colname)
            if colname != new_colname:
                self.glogger.error("failed to create a new column=%s in table=%s", colname, dbtable.get_table_name())
                return False
        except Exception as err:
            self.glogger.error("Exception to create a new column=%s in table=%s err=%s", colname, dbtable.get_table_name(), err)
            return False
        return True