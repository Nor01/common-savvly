...
class DbUserTables(DbTableCollection):
    ...
    #tab_name_inherit = "inherited"                 # User Inheritence
    ...
    #col_name_depositedamount = "depositedamount"
    col_name_numshares = "numshares"
    col_name_shareprice = "shareprice"
    #col_name_redistributedamount = "redistributedamount"
    ...
    def _build_tables_info(self):
        ...
        #tables_info[self.tab_name_pii] =  [
        #                          (self.col_name_userid, "text PRIMARY KEY"),
        #                          (self.col_name_social, "text"),
        #                          (self.col_name_dateofbirth, "text"),
        #                          (self.col_name_address, "text"),
        #                          ]
        ...
        #                          (self.col_name_userid, "text PRIMARY KEY"),
        #                          (self.col_name_depositedamount, "text"),
        #                          (self.col_name_numshares, "text"),
        ...
        #                          (self.col_name_userid, "text PRIMARY KEY"),
        #                          (self.col_name_redistributedamount, "text"),
        #                          (self.col_name_numshares, "text"),
        #                          (self.col_name_shareprice, "text"),
        #                          ]
        return tables_info
    ...
    # Add new user
    # -----------------------------------
    def add_new_user(self, idx:str, social:str, date_of_birth:str, address:str, sex:str, mother_name:str):
        if len(social) < 4:
            self.glogger.error("The SS (%s) value is invalid. It should bt at least 4 characters", social)
            return False
        social_4chars = social[-4:]

        dob = datetime_parse(date_of_birth)
        if dob is None:
            return False
        age = datetime_calc_age(dob)
        if age < 0:
            self.glogger.error("The date of birth is invalid (%s->%s). The age must be positive (%d)", date_of_birth, dob, age)
            return False
        if age > 120:
            self.glogger.error("The date of birth is invalid (%s->%s). The age exceeds the allowed range (%d)", date_of_birth, dob, age)
            return False
        ...
        if sex != 'F' and sex != 'M':
            ...
            return False
        self.glogger.info("Adding IDX=%s social=%s dob=%s (age=%d) address=%s sex=%s", idx, social,dob, age, address, sex)
        ...
        values[self.col_name_lastupdate] = self._get_last_update_time()
    ...
    def _is_status_flag_change_allowed(self, cur_status:str, new_status:str):
        ...
        if result:
            ...
            self.glogger.error("Table: It is not allowed to change the status flag from %s to %s", cur_status,new_status)
        return result
    ...
    def _update_userdata_values(self, idx: str, data_dict: dict):
        if self.col_name_accountid in data_dict:
            ...
            return False
        data_dict[self.col_name_lastupdate] = self._get_last_update_time()
    ...
    def get_user_pii(self, idx: str) -> dict:
        ...
        try:
            ...
            self.glogger.error("Could not convert the PII info of account %s to dictionary:%s err=%s", accountid, pii_str, err)
        return None
    ...
    def get_inherited_shares(self, idx: str) -> float:
        ...
        try:
            ...
            self.glogger.error("The format of inherited_shares is invalid (idx=%s): %s err=%s", idx, inherited_shares_str, err)
            return 0.0
    ...
    # Update Status Flag
    # ------------------------------------------
    def set_statusflag_active(self, idx:str):  return self._update_status_flag(idx, self.status_flag_active)
    def set_statusflag_pending(self, idx:str):  return self._update_status_flag(idx, self.status_flag_pending)
    def set_statusflag_transfer(self, idx:str):  return self._update_status_flag(idx, self.status_flag_transfer)
    def set_statusflag_transfer_complete(self, idx:str):  return self._update_status_flag(idx, self.status_flag_transfer_complete)
    def set_statusflag_purchase_pending(self, idx:str):  return self._update_status_flag(idx, self.status_flag_purchase_pending)
    def set_statusflag_withdrawal(self, idx:str):  return self._update_status_flag(idx, self.status_flag_withdrawal)
    def set_statusflag_withdrawal_pending(self, idx:str):  return self._update_status_flag(idx, self.status_flag_withdrawal_pending)
    def set_statusflag_deceased(self, idx:str):  return self._update_status_flag(idx, self.status_flag_deceased)
    def set_statusflag_closed(self, idx:str):  return self._update_status_flag(idx, self.status_flag_closed)
    def set_statusflag_transfer_cancel(self, idx:str):  return self._update_status_flag(idx, self.status_flag_transfer_cancel)
    ...
    def update_inherited_shares(self, idx:str, inherited_shares : float):
        ...
        self.glogger.info("Changing inherited shares from %f to %f for user %s", cur_inherited_shares, inherited_shares, idx)
        data_dict = {}
    ...
    # # ------------------------------------------
    def _transact_money(self, idx:str, amount:float):
        ...
        if cur_status != self.status_flag_active:
            self.glogger.error("The status flag (idx=%s) is not Active (%s). Money Transfer is not allowed", idx, cur_status)
            return False
        ...
        try:
            ...
            self.glogger.error("The transfer money in the db (idx=%s) is not a float number:%s err=%s", idx, cur_amount_str, err)
            return False
        if cur_amount != 0:
    ...
    # ------------------------------------------
    def deposit_money(self, idx: str, amount: float):
        try:
            if amount <= 0:
                self.glogger.error("The amount of money (%s) to deposit is invalid (id=%s)", str(amount), idx)
                return False
        except Exception as err:
            self.glogger.error("Invalid amount of money to do transaction with (idx=%s):%s err=%s", idx, str(amount), err)
            return False
        return self._transact_money(idx, amount)
    ...
    def set_transfer_complete(self, idx:str):
        ...
        if cur_status != self.status_flag_transfer:
            self.glogger.error("The status flag (idx=%s) is not Transfer (%s). No money is in transaction now", idx, cur_status)
            return False
    ...
    def set_purchase_complete(self, idx:str, share_price : float) -> float:
        ...
        if cur_status != self.status_flag_purchase_pending:
            self.glogger.error("The status flag (idx=%s) is not purchase-pending (%s). No money is in transaction now", idx, cur_status)
            return 0.0 # Error
        ...
        if not result:
            self.glogger.error("Failed to complete the purchase: idx=%s amount=%s shareprice=%s", idx, userdata[self.col_name_transferamount], share_price)
            return 0.0  # Error
        return amount
    ...
    def set_withdrawal_complete(self, idx:str, share_price : float):
        ...
        if cur_status != self.status_flag_withdrawal_pending:
            self.glogger.error("The status flag (idx=%s) is not withdrawal-pending (%s). No money is in transaction now", idx, cur_status)
            return 0.0 # Error
        amount = float(userdata[self.col_name_transferamount])
        if amount >= 0.0:
            self.glogger.error("Invalid amount: idx=%s shareprice=%s amount=%f", idx, share_price, amount)
        ...
        result = self._update_userdata_values(idx, data_dict)
        if not result:
            self.glogger.error("Failed to complete the withdrawal: idx=%s amount=%s shareprice=%s", idx, userdata[self.col_name_transferamount], share_price)
            return 0.0  # Error
        return amount
    ...
    # Calculate the
    # ------------------------------------------
    def _calc_num_shares_after_transaction(self, share_price : float, amount_str:str, cur_num_shares_str:str)-> (str,str):
        try:
            ...
            self.glogger.error("Invalid format in one of the values: shareprice=%s amount=%s num_shares=%s err=%s", str(share_price), amount_str, cur_num_shares_str, err)
    ...
    def get_all_parentids(self):
        value_list = self.get_table_data_per_colnames(self.tab_name_data, [self.col_name_userid, self.col_name_parentid])
        #print(value_list)
        return value_list
    ...
    def get_all_accountids(self):
        value_list = self.get_table_data_per_colnames(self.tab_name_data, [self.col_name_userid, self.col_name_accountid])
        #print(value_list)
        return value_list
    ...
    def get_all_statuses(self):
        value_list = self.get_table_data_per_colnames(self.tab_name_data, [self.col_name_userid, self.col_name_statusflag])
        #print(value_list)
        return value_list
    ...
    def _calc_sum_shares_dp(self, all_users_values:list) -> (float,float):
        ...
        for values in all_users_values:
            ...
            try:
                ...
                return 0.0, 0.0
            self.glogger.debug("idx=%s age=%d sex=%s num_shares=%f statusflag=%s dp=%f", idx, age, sex, numshares, statusflag, dp)
    ...
    # ------------------------------------------------
    def _normalize_shares_dp(self, distribute_num_shares:float, all_users_values:list, dp_sum : float, shares_sum : float) -> list:
        ...
        self.glogger.info("total_shares_per_dp=%f total_shares_transfer=%f total_shares_transfer_percent=%f ", total_shares_per_dp, total_shares_transfer, total_shares_transfer_percent)
    ...
    def calculate_dead_user_shares_distribution(self, idx:str) -> list:
        ...
        if status_flag != self.status_flag_deceased:
            self.glogger.error("The status of the user %s is not deceased (it is %s). Cannot distribute its shares", idx, status_flag)
            return None
    ...
    # Get all records with a specific flag value
    # ------------------------------------------
    def get_all_statusflag_pending(self): return self._get_all_specific_statusflag(self.status_flag_pending)
    def get_all_statusflag_active(self): return self._get_all_specific_statusflag(self.status_flag_active)
    def get_all_statusflag_transfer(self): return self._get_all_specific_statusflag(self.status_flag_transfer)
    def get_all_statusflag_transfer_complete(self): return self._get_all_specific_statusflag(self.status_flag_transfer_complete)
    def get_all_statusflag_purchase_pending(self): return self._get_all_specific_statusflag(self.status_flag_purchase_pending)
    def get_all_statusflag_withdrawal(self): return self._get_all_specific_statusflag(self.status_flag_withdrawal)
    def get_all_statusflag_withdrawal_pending(self): return self._get_all_specific_statusflag(self.status_flag_withdrawal_pending)
    def get_all_statusflag_deceased(self): return self._get_all_specific_statusflag(self.status_flag_deceased)
    def get_all_statusflag_closed(self): return self._get_all_specific_statusflag(self.status_flag_closed)
    def get_all_statusflag_transfer_cancel(self): return self._get_all_specific_statusflag(self.status_flag_transfer_cancel)

    # ------------------------------------------
    # Get All  FMVs
    # ------------------------------------------
    def get_all_fmvs(self):
        value_list = self.get_table_data_per_colnames(self.tab_name_data, [self.col_name_userid, self.col_name_fair_market_value])
        #print(value_list)
        return value_list
    ...
    def get_num_shares(self, idx: str) -> float:
        ...
        try:
            ...
            self.glogger.error("The Num of shares in the db (idx=%s) is not a float number:%s err=%s", idx, num_shares_value_str, err)
            return -1
    ...
    def _update_fmv(self, idx:str, share_price : float):
        ...
        try:
            ...
            self.glogger.error("The Num of shares in the db (idx=%s) is not a float number:%s err=%s", idx, num_shares_str, err)
            return False
        if num_shares < 0:
        ...
        try:
            ...
            self.glogger.error("The FMV in the db (idx=%s) is not a float number:%s fund_value=%f err=%s", idx, current_fmv_str, fund_value, err)
            return False

        self.glogger.info("updating fmv: share_price=%f num_shares=%f current_fmv=%f", share_price, num_shares, current_fmv)
