...
def CheckForNewSignedContracts():
    ...
    glogger.debug("CheckForNewSignedContracts")
    ...
    glogger.debug(f"found {len(sent_contracts)} sent contracts")
    ...
    for contract in sent_contracts:
        ...
        info = docusign_helper.contract_info()
        ...
        if info:
            ...
            if signed_date is not None:
                ...
                glogger.info(
                    f"found contract signed date {signed_date} for {client_info['email']} updating status to Sent")
                ...
                try:
                    # client_info['contractid'] = contract_id
                    client_info['contracturl'] = docusign_helper.get_contract_url()
                    # client_info['advisorid'] = contract['advisorid']
                    # client_info['advisoridx'] = contract['idx']
                except Exception as err:
                    glogger.error("Failed to add Contract info to the client Info. err=%s", err)  # pass the error
...
class DocuSignContractHelper:
    ...
    def get_contract_url(self):
        try:
            return self.docusign_service_retrieve_url.format(envelope=self.contract_id)
        except Exception as err:
            self.glogger.error("Exception to get the url of the contractid=%s. err=%s", self.contract_id, err)
            return None

    # retrieve contract's pdf from docusign, assumes there is a single contract in the envelope
    def retrieve_contract(self):
        try:
            url = self.get_contract_url()
            ...
            self.glogger.error("Exception to retrive the contract from url=%s id=%s. err=%s", url, self.contract_id,
                               err)
            return None
    ...
    def contract_info(self):
        try:
            ...
            self.glogger.debug(f"getting info for Docusign contract {self.contract_id}")
            ...
            self.glogger.error("Exception to retrive contract info for id=%s. err=%s", self.contract_id, err)
            return None
