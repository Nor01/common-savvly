...
class AzureFileStorage:
    ...
    def create_share(self):
        try:
            ...
            self.glogger.debug("Creating share: %s", self.share_name)
            share_client.create_share()
            return True
        except ResourceExistsError as err:
            self.glogger.debug("The share (%s) already exists. err=%s", self.share_name, err)
            return True
        except Exception as err:
            #print("XXXXXXX" + str(type(err).__name__))
    ...
    def create_directory(self, dir_name):
        try:
            ...
            self.glogger.debug("Creating directory: %s/%s", self.share_name, dir_name)
            dir_client.create_directory()
            return True
        except ResourceExistsError as err:
            self.glogger.debug("The directory (%s) already exists. err=%s", dir_name, err)
            return True
        except Exception as err:
            #print("YYYYYY" + str(type(err).__name__))
        ...
        try:
            ...
            for item in list(share_client.list_directories_and_files(dir_name)):
                ...
                if item["is_directory"]:
                    self.glogger.debug("Directory: %s", name)
                else:
                    self.glogger.debug("File: %s/%s", dir_name, name)
    ...
    def upload_file(self, local_file_path, dest_file_path):
        try:
            ...
            self.glogger.info("Uploading %s to: %s/%s", local_file_path, self.share_name, dest_file_path)
    ...
    # ----------------------------------------------------------------------
    def upload_file_to_dir(self, local_file_path, dir_name, add_time_suffix : bool = False):
        try:
            basename = os.path.basename(local_file_path)
            dest_file_path = os.path.join(dir_name, basename)
            if add_time_suffix:
                timestr = time.strftime("%Y-%m-%d-%H%M%S")
                dest_file_path = dest_file_path + '.' + timestr
    ...
    # ----------------------------------------------------------------------
    def upload_log_file(self, local_file_path, add_time_suffix : bool = False):
        return self.upload_file_to_dir(local_file_path, self.folder_logs, add_time_suffix)
